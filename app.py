...
class DuckBrain:
    ...
    def start_session(self, bug: str) -> dict[str, Any]:
        ...
        stage = get_stage(1)
        message, source, note = self._generate_question(issue, stage, "", [])
        return {
            "ok": True,
            "kind": "question",
            "level": stage["level"],
    ...
    def continue_session(
        ...
        if current_level >= len(STAGES) - 1:
            ...
            message, source, note = self._generate_closure(issue, latest_answer, turns)
            return {
                "ok": True,
                "kind": "complete",
                "level": stage["level"],
        ...
        message, source, note = self._generate_question(issue, next_stage, latest_answer, turns)
        return {
            "ok": True,
            "kind": "question",
    ...
    def _generate_question(
        ...
        if self.api_key and self.response_mode == "gemini":
            try:
                ...
                return question, "gemini", None
            except Exception as exc:
                fallback = self._fallback_question(bug, stage, latest_answer, history)
                return fallback, "fallback", f"Gemini question generation failed: {exc}"

        note = "Fast template mode keeps questions immediate."
        if self.api_key and self.response_mode == "fast":
            note = "Fast template mode is active. Set RDEP_RESPONSE_MODE=gemini to use live Google AI questions."
        return self._fallback_question(bug, stage, latest_answer, history), "template", note
    ...
    def _generate_closure(
        ...
        return self._fixed_closure(), "template", "Fixed enterprise closure text is active."
    ...
    def _call_gemini(self, system_prompt: str, user_prompt: str) -> str:
        ...
        req = request.Request(
            ...
            headers={
                ...
                "x-goog-api-client": "rubber-duck-escalation/0.2",
            },
            method="POST",
        )
        try:
...
def main() -> None:
    port = int(os.getenv("RUBBER_DUCK_PORT", DEFAULT_PORT))
    server = ThreadingHTTPServer(("127.0.0.1", port), RubberDuckHandler)
    print(f"{APP_VERSION} running on http://127.0.0.1:{port}")
    try:
